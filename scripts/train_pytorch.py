...
import openpi.models_pytorch.framesamp_pytorch
...
def _is_expected_context_missing_key(model, key: str) -> bool:
    ...
    if isinstance(model_to_check, openpi.models_pytorch.framesamp_pytorch.PI0FramesampContextPytorch):
        return key.startswith("context_")
    return False
...
def train_loop(config: _config.TrainConfig):
    ...
    if isinstance(model_cfg, openpi.models.pi0_config.Pi0FramesampContextConfig):
        model = openpi.models_pytorch.framesamp_pytorch.PI0FramesampContextPytorch(model_cfg).to(device)
    else:
    ...
    if config.pytorch_weight_path is not None:
        ...
        load_pytorch_weights(
            model,
            model_path,
            allow_context_missing=isinstance(model, openpi.models_pytorch.framesamp_pytorch.PI0FramesampContextPytorch),
            device=device,
        )
